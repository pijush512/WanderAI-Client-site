...
export default function WanderEngine() {
  ...
  const fetchEngineLogs = async () => {
    try {
      ...
      if (!response.ok)
        throw new Error("Neural Core connection failed (Check Backend)");
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] text-rose-500">
        ...
        <button
          ...
          className="mt-6 px-6 py-2 bg-slate-900 dark:bg-blue-600 text-white rounded-xl text-xs font-bold uppercase tracking-widest transition-all"
        >
          Retry Connection
        </button>
      </div>
    ...
    <div className="space-y-10 transition-colors duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-slate-900 dark:bg-blue-600 p-8 rounded-[2.5rem] text-white relative overflow-hidden group transition-all">
          <Activity
            className="absolute right-[-10px] top-[-10px] text-white/5 group-hover:text-white/10 transition-colors"
            size={120}
          />
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm">
          ...
          <h3 className="text-4xl font-black italic text-blue-600 dark:text-blue-400 tracking-tighter uppercase">
            Llama-3.1
          </h3>
        </div>
      </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/50 dark:bg-slate-800/50">
                <th className="px-10 py-5 text-[9px] font-black uppercase tracking-widest text-slate-400">
                  Explorer
                </th>
                <th className="px-10 py-5 text-[9px] font-black uppercase tracking-widest text-slate-400">
                  Target Vector
                ...
                <th className="px-10 py-5 text-[9px] font-black uppercase tracking-widest text-slate-400 text-center">
                  Status
                </th>
              </tr>
            </thead>
              ...
              {loading
                ? [...Array(3)].map((_, i) => (
                    <tr key={i} className="animate-pulse">
                      <td
                        colSpan={4}
                        className="px-10 py-8 bg-slate-50/10 dark:bg-slate-800/10"
                      ></td>
                    </tr>
                  ))
                : logs.map((log) => (
                    ...
                    >
                      <td className="px-10 py-6">
                        <div className="flex flex-col">
                          ...
                          <span className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase italic">
                            Budget: {log.budget}
                          </span>
                        </div>
                      </td>
