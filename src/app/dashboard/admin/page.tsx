...
export default function AdminOverview() {
  ...
  const statsCards = [
    ...
    },
    {
      ...
      value: `${data?.activeRegions || 0} Cities`,
      icon: Globe,
    ...
    {
      label: "System Health",
      value: data?.serverStatus || "Operational",
      icon: Activity,
    ...
    <div className="p-4 md:p-8 space-y-10 bg-slate-50/50 dark:bg-slate-950 min-h-screen animate-in fade-in duration-1000">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {statsCards.map((stat, i) => (
          <div
            key={i}
            className={`bg-white dark:bg-slate-900 p-6 rounded-[2.5rem] border ${stat.border} dark:border-slate-800 shadow-sm transition-all hover:-translate-y-2 hover:shadow-xl group relative overflow-hidden`}
          >
            <div
              className={`absolute top-0 right-0 w-24 h-24 ${stat.bg} blur-3xl rounded-full -mr-10 -mt-10 group-hover:scale-150 transition-transform`}
            ></div>
            <div className="flex justify-between items-start mb-6 relative z-10">
              <div
                className={`p-4 rounded-2xl ${stat.bg} dark:bg-slate-800 ${stat.color} ring-1 ring-inset ${stat.border}`}
              >
                <stat.icon size={22} />
              </div>
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 p-8 rounded-[3rem] border border-slate-200 dark:border-slate-800 shadow-sm relative group overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-10">
            ...
            <div className="flex gap-2">
              <div className="px-3 py-1 bg-slate-100 dark:bg-slate-800 rounded-full text-[9px] font-bold text-slate-500 uppercase tracking-tighter italic">
                Last 7 Days
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4 flex-1 z-10 overflow-y-auto max-h-[380px] custom-scrollbar pr-2">
            {data?.recentLogs && data.recentLogs.length > 0 ? (
              data.recentLogs.map((log, i) => (
                <div
                  key={i}
                  className="flex gap-4 items-center p-4 bg-white/5 rounded-[1.5rem] border border-white/5 hover:border-indigo-500/50 hover:bg-white/10 transition-all group backdrop-blur-sm"
                >
                  <div className="relative">
                    <div className="w-12 h-12 rounded-2xl bg-indigo-600 flex items-center justify-center font-black text-sm shrink-0 shadow-lg group-hover:rotate-6 transition-transform overflow-hidden">
                      {log.user?.image ? (
                        <img
                          src={`${process.env.NEXT_PUBLIC_BASE_URL}/${log.user.image}`}
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-xs leading-tight font-black tracking-tight">
                      <span className="text-indigo-400 group-hover:text-indigo-300 transition-colors uppercase italic">
                        {log.user?.name || "User"}
                      </span>
                      <span className="opacity-40 font-medium px-2 italic">
                        deployed to
                      </span>
