...
// import { 
//   LayoutDashboard, Users, Map, Star, 
//   Settings, LogOut, Menu, X, Shield, Globe,
//   Home, User, Compass, FileText, Activity
// } from "lucide-react";
...
//   // ১. মেইন সাইডবার (একেবারে বামে - Primary Navigation)
//   const primaryNav = [
...
//   // ২. সেকেন্ডারি সাইডবার (Admin Core মেনু)
...
//     { name: "System Config", icon: Settings, path: "/dashboard/admin/ai-config" },
//   ];
...
//             // চেক করা হচ্ছে আপনি এই সেকশনে আছেন কি না
...
//            <button onClick={logout} className="p-3 text-slate-500 hover:text-rose-500 transition-colors">
//               <LogOut size={22} />
//            </button>
//         </div>
//       </aside>

//       {/* --- স্তর ২: সেকেন্ডারি সাইডবার (ADMIN CORE) --- */}
//       <aside className={`fixed lg:relative inset-y-0 left-0 z-50 w-72 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 transition-transform duration-300 transform ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}>
//         <div className="flex flex-col h-full p-6 pt-10">
          
//           <div className="px-2 mb-12">
//             <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Operational Area</span>
//             <h1 className="text-xl font-black dark:text-white uppercase tracking-tighter italic mt-1">Admin<span className="text-blue-600">Core</span></h1>
//           </div>

//           <nav className="flex-1 space-y-2">
//             {adminMenuItems.map((item) => {
//               // Active status logic: পাথ মিলে গেলে একটিভ দেখাবে
...
//                   <item.icon 
//                     size={18} 
//                     className={`${isActive ? "text-blue-600" : "group-hover:scale-110 transition-transform"}`} 
...
//           <div className="hidden md:flex items-center gap-2 px-4 py-1.5 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 rounded-full text-[9px] font-black uppercase tracking-widest">
...
//                 <p className="text-xs font-black dark:text-white uppercase leading-none">Admin User</p>
//                 <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">System Root</p>
//              </div>
//              <div className="w-10 h-10 rounded-full bg-slate-200 dark:bg-slate-800 border-2 border-white dark:border-slate-700"></div>
//           </div>
//         </header>
...
export default function AdminLayout({
  ...
  const [isSidebarOpen, setIsSidebarOpen] = useState(true);
  const pathname = usePathname();
  const { logout, user } = useAuth(); // user অবজেক্টটি এখান থেকে আসছে
            ...
            <div className="w-10 h-10 rounded-full overflow-hidden bg-slate-100 dark:bg-slate-800 border-2 border-white dark:border-slate-700 shadow-sm flex items-center justify-center">
              {user?.image ? (
                <img
                  ...
                  onError={(e) => {
                    // যদি ইমেজ লোড হতে এরর দেয়, তবে এটি আইকন দেখাবে
              ...
              {/* Fallback আইকন যদি ইমেজ না থাকে */}
