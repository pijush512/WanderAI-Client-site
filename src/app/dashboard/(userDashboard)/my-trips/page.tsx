...
import { 
  ...
  Calendar, Filter, ChevronLeft, ChevronRight, Check,
    ...
    <div className="p-4 md:p-8 space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm font-medium">Managing your travel history for Tangail & beyond.</p>
        </div>
        <Link 
        ...
        <div className="relative" ref={filterRef}>
          <button 
            ...
            className={`flex items-center gap-2 px-5 py-3 bg-white dark:bg-slate-900 border rounded-2xl font-bold transition-all text-sm ${
              budgetFilter !== "all" ? "border-blue-600 text-blue-600 shadow-lg shadow-blue-500/10" : "border-slate-200 dark:border-slate-800 text-slate-600"
            }`}
          >
            <Filter size={18} /> 
            {budgetFilter === "all" ? "All Budgets" : `${budgetFilter.toUpperCase()}`}
          </button>
          ...
          {isFilterOpen && (
            <div className="absolute right-0 mt-2 w-52 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl shadow-2xl z-50 p-2 space-y-1 animate-in zoom-in-95">
              {[
                ...
                <button 
                  key={opt.id}
                  onClick={() => { setBudgetFilter(opt.id as any); setIsFilterOpen(false); }}
                ...
                >
                  {opt.label}
                  {budgetFilter === opt.id && <Check size={14} className="text-blue-600" />}
                </button>
              ))}
            </div>
          )}
              ...
              {processedTrips.length > 0 ? (
                  ...
                  <tr key={trip._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-black text-slate-900 dark:text-white uppercase tracking-tight leading-none mb-1.5">{trip.destination}</p>
                          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
                            <Calendar size={12} className="text-blue-500" /> {new Date(trip.createdAt).toLocaleDateString()}
                          </p>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-5">
                      <div className="space-y-1.5">
                        <div className="flex items-center gap-2 text-xs font-black text-slate-600 dark:text-slate-400 uppercase tracking-tighter">
                          <Clock size={14} className="text-slate-400" /> {trip.days} Days
                        </div>
                        <div className="flex items-center gap-2 text-xs font-black text-slate-600 dark:text-slate-400 uppercase tracking-tighter">
                          <Users size={14} className="text-slate-400" /> {trip.travelers} Traveler(s)
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-5">
                      <span className={`px-4 py-2 border rounded-full text-[9px] font-black uppercase tracking-widest flex items-center gap-1.5 w-fit shadow-sm ${getBudgetStyle(trip.budget)}`}>
                        <DollarSign size={12} /> {trip.budget}
                      </span>
                    </td>
                    <td className="px-8 py-5">
                      <div className="flex items-center justify-center gap-3">
                        <Link href={`/dashboard/my-trips/${trip._id}`} className="p-2.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-xl transition-all shadow-sm bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
                          <Eye size={18} />
                        </Link>
                        <button onClick={() => handleDelete(trip._id)} className="p-2.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-xl transition-all shadow-sm bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
                          <Trash2 size={18} />
                        </button>
                      </div>
                    </td>
                ...
                <tr>
                  <td colSpan={4} className="px-8 py-24 text-center">
                    <p className="text-slate-300 font-black uppercase text-xs tracking-[0.2em]">No trips found</p>
                  </td>
                </tr>
              )}
            </tbody>
