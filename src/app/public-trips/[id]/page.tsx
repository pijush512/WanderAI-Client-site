    ...
    const fetchDetails = async () => {
      try {
        ...
        const response = await axios.get(
          `https://wander-ai-server-site.vercel.app/api/v1/ai/all-trips`,
        );
        if (response.data.success) {
          const foundTrip = response.data.data.find(
            (t: any) => t._id === params.id,
          );
          setTrip(foundTrip);
        }
      } catch (err) {
      ...
      <div className="relative h-[70vh] w-full bg-slate-900 flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-6 max-w-4xl space-y-8">
          ...
          <p className="text-white/60 font-medium italic text-sm md:text-lg leading-relaxed max-w-2xl mx-auto">
            Welcome to your personalized guide for{" "}
            <span className="text-white font-bold not-italic underline decoration-blue-500 underline-offset-4">
              {trip.destination}
            </span>
            . Our AI has curated a{" "}
            <span className="text-white font-bold">{trip.days}-day</span>{" "}
            adventure optimized for a{" "}
            <span className="text-white font-bold">{trip.budget}</span> budget.
          </p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-3xl mx-auto">
            {[
              ...
              <div
                key={i}
                className="bg-white/5 backdrop-blur-sm border border-white/10 p-4 rounded-3xl text-center"
              >
            ...
            <div className="lg:col-span-8 space-y-20">
              ...
              <div className="space-y-12">
                {trip.itinerary?.map((item: any, idx: number) => (
                  <div
                    key={idx}
                    className="group bg-slate-50 dark:bg-slate-800/30 rounded-[3.5rem] p-10 border border-slate-100 dark:border-slate-800 transition-all hover:border-blue-200 dark:hover:border-blue-900"
                  >
                      ...
                      <div className="space-y-4">
                        ...
                        <ul className="space-y-3">
                          {item.activities?.map((act: string, i: number) => (
                            <li
                              key={i}
                              className="flex items-start gap-3 text-sm font-bold text-slate-700 dark:text-slate-300 italic"
                            >
                              <CheckCircle2
                                size={16}
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {item.locations?.map((loc: string, i: number) => (
                            <span
                              key={i}
                              className="px-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl text-[10px] font-black uppercase text-slate-500 hover:text-blue-600 transition-colors"
                            >
                              {loc}
                            </span>
                          ))}
              ...
              <div className="py-20 bg-slate-900 dark:bg-blue-600 rounded-[4rem] text-center px-10 space-y-8 text-white">
                ...
                <div className="flex justify-center gap-2">
                  {[1, 2, 3, 4, 5].map((s) => (
                    <Star
                      key={s}
                      size={30}
                      className="text-white/20 hover:text-yellow-400 cursor-pointer transition-colors"
                    />
                  ))}
                </div>
                ...
                <div className="pt-8 border-t border-white/10">
                  ...
                  <p className="text-sm italic opacity-40 uppercase tracking-widest font-bold">
            ...
            <div className="lg:col-span-4 space-y-12">
              ...
              <div className="p-10 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-[3.5rem] shadow-2xl">
                <h4 className="flex items-center gap-3 font-black uppercase text-[10px] tracking-widest mb-8 border-b border-white/10 dark:border-slate-200 pb-4">
                  <Lightbulb className="text-yellow-400" size={16} /> Travel
                  Tips
                </h4>
                <div className="space-y-6">
                  {trip.travel_tips?.map((tip: string, i: number) => (
                    <div key={i} className="flex gap-4 items-start">
                      <div className="w-6 h-6 rounded-full bg-blue-600 flex items-center justify-center text-[10px] font-black shrink-0">
                        {i + 1}
                      </div>
              ...
              <div className="space-y-8 pt-8">
                ...
                <div className="grid grid-cols-1 gap-4">
                  {["Tangail", "Sylhet", "Sajek Valley"].map((place, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-5 p-5 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl hover:border-blue-500 transition-all cursor-pointer group"
                    >
              ...
              <div className="p-10 bg-rose-50 dark:bg-rose-900/10 border border-rose-100 dark:border-rose-900/30 rounded-[3.5rem] space-y-4">
                ...
                <button className="text-[10px] font-black uppercase text-rose-600 hover:text-rose-700 transition-colors">
