...
//       {/* ১. লেফট সাইড - সার্চ বার (Search Bar) */}
...
//       {/* ২. রাইট সাইড - নোটিফিকেশন ও প্রোফাইল ড্রপডাউন */}
//       <div className="flex items-center gap-3">
        
//         {/* থিম টগল বা নোটিফিকেশন (Optional) */}
...
export default function DashboardNavbar() {
  ...
  // ১. সার্চের জন্য স্টেট
  const [searchQuery, setSearchQuery] = useState("");

  // ২. সার্চ হ্যান্ডলার ফাংশন
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      // আপনার সার্চ রেজাল্ট পেজের রাউট এখানে দিন
  ...
  return (
    <header className="h-20 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-6 flex items-center justify-between sticky top-0 z-50">
      ...
      {/* ২. রাইট সাইড - নোটিফিকেশন ও প্রোফাইল */}
      <div className="flex items-center gap-3">
        <button className="p-2.5 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-all relative">
          <Bell size={20} />
          <span className="absolute top-2 right-2 w-2 h-2 bg-red-500 rounded-full border-2 border-white dark:border-slate-900"></span>
